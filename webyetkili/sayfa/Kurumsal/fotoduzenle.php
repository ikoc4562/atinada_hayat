...
if ($_FILES["fotograf"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE kurumsalgaleri SET fotograf = :foto WHERE id = :id");
...
$ozet_eng = str_replace("'","`",$ozet_eng);
			...
			if($anasayfa == ""){
 			...
 			$db->query("UPDATE kurumsalgaleri SET baslik='".$baslik."',baslik_eng='".$baslik_eng."',aktif='".$aktif."',anasayfa='".$anasayfa."' WHERE id = '".$_GET['id']."'");

$mesaj = 'başarılı';
}

$ssql = $db->prepare("SELECT * FROM kurumsalgaleri WHERE id = :id");
...
?>
 ...
 <meta http-equiv="refresh" content="3;URL=index.php?page=Kurumsal&git=Fotograf&id=<?=$_GET['sid']?>" />
...
<input name="anasayfa" type="checkbox" class="form-control tooltips" id="anasayfa" data-trigger="hover" data-toggle="tooltip" value="1" <?php if($upcek[0]['anasayfa'] == "1"){echo "checked";}?>/>
     </div></div>
