...
if ($_FILES["fotograf"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE ziyaretlergaleri SET fotograf = :foto WHERE id = :id");
...
$ozet_eng = str_replace("'","`",$ozet_eng);
			...
			if($aktif == ""){
 			...
 			$db->query("UPDATE ziyaretlergaleri SET baslik='".$baslik."',baslik_eng='".$baslik_eng."',aktif='".$aktif."' WHERE id = '".$_GET['id']."'");

$mesaj = 'başarılı';
}

$ssql = $db->prepare("SELECT * FROM ziyaretlergaleri WHERE id = :id");
...
?>
 ...
 <meta http-equiv="refresh" content="3;URL=index.php?page=Ziyaretler&git=Fotograf&id=<?=$_GET['id']?>" />
