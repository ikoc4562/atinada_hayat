...
<?php }?>
    ...
    $sql2->execute(['id'=>$_GET['id']]);
        ...
        <div class="panel-body">
          ...
          <a href="index.php?page=Odatipleri&git=yeniFoto&id=<?=$_GET['id']?>" class="btn btn-primary btn-lg"> Lisää Uusi Kuva </a>
           ...
           <td></td>
             <td><img src="../kucult.php?w=200&img=../images/Odalar/<?=$cek['fotograf']?>" style="padding-right:10px;" /><?=$cek['baslik']?></td>
                  ...
                  <td class="hidden-phone">
                   <?php if($cek['sira'] != "1"){ ?>
   			      <a href="index.php?page=Odatipleri&git=Fotograf&id=<?=$_GET['id']?>&islem=yukari&aid=<?=$cek['id']?>&sira=<?=$cek['sira']?>" title="Yukarı Taşı" class="btn btn-default active"><i class="fa fa-arrow-circle-up"></i></a>
				  <?php } if ($cek['sira'] != $kactanecik){?>
   				  <a href="index.php?page=Odatipleri&git=Fotograf&id=<?=$_GET['id']?>&islem=asagi&aid=<?=$cek['id']?>&sira=<?=$cek['sira']?>" title="Aşağı Taşı" class="btn btn-default active"><i class="fa fa-arrow-circle-down"></i></a>
				  <?php }?>

	<a href="index.php?page=Odatipleri&git=FotoDuzenle&id=<?=$cek['id']?>&sid=<?=$_GET["id"]?>" title="Düzenle" class="btn btn-default active">   <i class="fa fa-gear"></i></a>
