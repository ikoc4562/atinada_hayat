...
if ($_FILES["fotograf"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urundosya SET fotograf = :foto WHERE id = :id");
...
$ozet_eng = str_replace("'","`",$ozet_eng);
			...
			if($aktif == ""){
 			...
 			$db->query("UPDATE urundosya SET baslik='".$baslik."',baslik_eng='".$baslik_eng."',aktif='".$aktif."' WHERE id = '".$_GET['id']."'");

$mesaj = 'başarılı';
}

$ssql = $db->query("SELECT * FROM urundosya WHERE id = '".$_GET['id']."'");
$upcek = $ssql->fetchAll();
?>
 ...
 <meta http-equiv="refresh" content="3;URL=index.php?page=Urun&git=Dosya&id=<?=$upcek[0]['kat']?>" />
