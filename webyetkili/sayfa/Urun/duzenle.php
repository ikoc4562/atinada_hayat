...
if ($_FILES["fotograf"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf = :foto WHERE id = :id");
...
if ($_FILES["fotograf2"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf2 = :foto WHERE id = :id");
...
if ($_FILES["fotograf3"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf3 = :foto WHERE id = :id");
...
if ($_FILES["fotograf4"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf4 = :foto WHERE id = :id");
...
if ($_FILES["fotograf5"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf5 = :foto WHERE id = :id");
...
if ($_FILES["fotograf6"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf6 = :foto WHERE id = :id");
...
if ($_FILES["fotograf7"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf7 = :foto WHERE id = :id");
...
if ($_FILES["fotograf8"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf8 = :foto WHERE id = :id");
...
if ($_FILES["fotograf9"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf9 = :foto WHERE id = :id");
...
if ($_FILES["fotograf10"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf10 = :foto WHERE id = :id");
...
if ($_FILES["fotograf11"]["name"] != ""){
			...
			$sql = $db->prepare("UPDATE urun SET fotograf11 = :foto WHERE id = :id");
...
<label><font color="red">İçerik İngilizce</font></label>
 <textarea name="icerik_eng" class="ckeditor"><?=$upcek[0]['icerik_eng']?></textarea>  </div>
...
<select class="form-control m-bot15" name="kat" id="kat">
			...
			$icsqlbirim = $db->query("select * from urun WHERE kat = '0' order by id asc");
			$icabirim = $icsqlbirim->fetchAll();
			for ($ic=0; $ic < $icsqlbirim->rowCount(); $ic++) {

			if ($upcek[0]['kat'] == $icabirim[$ic]['id']) { $selected = " selected"; } else { $selected = ""; }
...
/*
			...
			if ($upcek[kat] == $icabirim[0][id]) { $selected = " selected"; } else { $selected = ""; }
echo "			<option value=\"".$icabirim[0]['id']."\"$selected>&nbsp;&nbsp;&nbsp;- ".$icabirim[0]['baslik']."</option>\n";
            }
*/
			?>
...
<div class="col-sm-12">
  <label> Yönlendir (URL)</label><input type="text" name="yonlendir" id="yonlendir" value="<?=$upcek[0]['yonlendir']?>" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip"/>
</div>

  <div class="square-green single-row"><div class="checkbox ">
<label>Yönlendirme Ayrı Pencerede Açılsın</label>
<input class="form-control checkbox" data-trigger="hover" data-toggle="tooltip" name="target" id="target" type="checkbox" value="1" <?php if($upcek[0]['target'] == "1"){echo "checked";}?>/></div></div>

  <div class="square-green single-row"><div class="checkbox "><label>Üst Menüde Göster</label>
<input class="form-control checkbox" data-trigger="hover" data-toggle="tooltip" name="anasayfa" id="anasayfa" type="checkbox" value="1" <?php if($upcek[0]['anasayfa'] == "1"){echo "checked";}?>/></div></div>


 <div class="square-green single-row"><div class="checkbox "><label>İlk Sayfa Yap (Giriş Sayfası)</label>
<input class="form-control checkbox" data-trigger="hover" data-toggle="tooltip" name="ilk" id="ilk" type="checkbox" value="1" <?php if($upcek[0]['ilk'] == "1"){echo "checked";}?>/></div></div>
...
<label>İçerik</label>
 <textarea name="icerik" class="ckeditor"><?=$upcek[0]['icerik']?></textarea>  </div>
             ...
             <div class="col-sm-12">
               <label>Include Dosyası 1 (Örn; fotoalbum.php gibi)</label><input type="text" name="include1" id="include1" value="<?=$upcek[0]['include1']?>" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip"/>
  </div>

    <div class="col-sm-12">
               <label>Include Dosyası 2</label><input type="text" name="include2" id="include2" value="<?=$upcek[0]['include2']?>" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip"/>
  </div>

    <div class="col-sm-12">
               <label>Include Dosyası 3</label><input type="text" name="include3" id="include3" value="<?=$upcek[0]['include3']?>" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip"/>
  </div>
