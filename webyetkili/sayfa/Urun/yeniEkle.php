...
$kac = $db->query("SELECT * FROM urun");
$say = $kac->rowCount();
// son sıra sayic
$sonsayi = $say + "1";

if ($_POST['islem'] == "urunekle"){
...
$sql = $db->query("INSERT INTO urun (baslik,kat,yonlendir,target,icerik,include1,include2,include3,aktif,anasayfa,sira,baslik_eng,icerik_eng,yonlendir_eng,ilk,fotograf,fotograf2,fotograf3,fotograf4,fotograf5,fotograf6,fotograf7,fotograf8,fotograf9,fotograf10,gosterim) VALUES ('".$baslik."','".$kat."','".$_POST['yonlendir']."','".$target."','".$icerik."','".$_POST['include1']."','".$_POST['include2']."','".$_POST['include3']."','".$aktif."','".$anasayfa."','".$sonsayi."','".$baslik_eng."','".$icerik_eng."','".$_POST['yonlendir_eng']."','".$ilk."','".$resim."','','','','','','','','','',0)");
...
/*
					$icsqlbirim = $db->prepare("select * from urunkategori WHERE kat = :kat order by id asc");
					$icsqlbirim->execute(['kat'=>$icabirim[0]['id']]);
					$icabirim = $icsqlbirim->fetchAll();
			for ($i=0; $i < $icsqlbirim->rowCount(); $i++) {

			if ($linebirim[0]['durum'] == $icabirim[$i]['id']) { $selected = " selected"; } else { $selected = ""; }
echo "			<option value=\"".$icabirim[$i]['id']."\"$selected>&nbsp;&nbsp;&nbsp;- ".$icabirim[$i]['baslik']."</option>\n";
            }
						*/
  }
			?>
...
<div class="col-sm-12">
<input name="islem" type="hidden" id="islem" value="urunekle" />
