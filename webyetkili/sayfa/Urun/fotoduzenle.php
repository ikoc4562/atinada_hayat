...
if ($_FILES["fotograf"]["name"] != ""){
			...
			$kaynak = "../images/urun/".$_GET['resimadi'];
			@unlink($kaynak);
			$hedef	= "../images/urun";
			...
			$sql = $db->prepare("UPDATE urungaleri SET fotograf = :foto WHERE id = :id");
...
if($aktif == ""){
	$aktif = 0;
 			...
 			$db->query("UPDATE urungaleri SET baslik='".$baslik."',baslik_eng='".$baslik_eng."',aktif='".$aktif."' WHERE id = '".$_GET['id']."'");

$mesaj = 'başarılı';
}

$ssql = $db->prepare("SELECT * FROM urungaleri WHERE id = :id");
...
?>
 ...
 <meta http-equiv="refresh" content="3;URL=index.php?page=Urun&git=Fotograf&id=<?=$_GET['id']?>" />
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
 ...
 <div class="col-sm-12">
  <label> Açıklama</label><input type="text" name="baslik" id="baslik" value="<?=$upcek[0]['baslik']?>" class="form-control tooltips" data-trigger="hover" data-toggle="tooltip"/>
</div>

 <div class="square-green single-row"><div class="checkbox ">
  <label>Aktif</label>
<input class="form-control tooltips" data-trigger="hover" data-toggle="tooltip" name="aktif" id="aktif" type="checkbox" value="1" <?php if($upcek['aktif'] == "1"){echo "checked";}?>/>
     </div></div>
